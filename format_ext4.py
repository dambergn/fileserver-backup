...
def format_device(device, label):
    try:
        ...
        subprocess.run(['sudo', 'mkfs.ext4', '-L', label, partition_path], check=True)

        print(f"Device {partition_path} formatted to ext4 with label '{label}'")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: sudo python3 format_ext4.py <device> <label>")
        print("Example: sudo python3 format_ext4.py sdb MyLabel")
        sys.exit(1)
